...
5. **✅ 结局（2026-09-17 18:07）：加白 `112.96.55.157` 后一次通过**，`errcode=0`，草稿入箱。验证结论三条：① 幂等守卫在失败时不落盘 hash，**原样重跑安全**，且成功后重跑会输出「相同内容已生成过草稿；为避免重复已停止」，草稿箱里确实只有一份；② **查出口 IP 不能只看 `api.ipify.org`**——它可能返回 IPv6（本机有 IPv6 出口），而 `api.weixin.qq.com` 只有 A 记录没有 AAAA，微信侧永远看到 IPv4；用 `curl -4 https://ipv4.icanhazip.com` 才准（`whatismyip.akamai.com` 会给出另一个值，疑似被代理/CDN 影响，别采信）；③ `/draft/batchget` **必须 POST**（GET 只回 errcode 不报错也不给数据），`/draft/count` 才是 GET。

---

## 坑 11：反查草稿时「正文与本地不一致」是假警报

**现象**：`draft/get` 拉回正文，与本地 `article-wechat.html` 逐字比对，报「不一致」，但去标签后字数完全相同（4290 = 4290），相似度 0.99。

**真因**：微信入库时会做四次规范化，全部无损：
1. 剥掉外层 `<!DOCTYPE>` / `<html>` / `<head>` / `<body ...>`；
2. 单引号转义成 `&#39;`（字体名里的 `'Helvetica Neue'` 之类首当其冲）；
3. `<br>` 归一成 `<br/>`；
4. 剥掉结尾 `</body></html>`。

**教训**：
1. 反查时**不要做逐字比对**，先做「去标签 + 去空白」归一化，再看差异是否只落在上面四类；相似度 0.99 且字数相同基本就是安全的。
2. `show_cover_pic` 返回 `0` **不是 bug**：新版 `draft/add` 已不列该字段，封面改由 `cover_info.crop_percent_list`（`2.35_1` 与 `1_1` 两套裁剪）控制。判定封面是否配好，看 `thumb_url` 非空 + `cover_info` 有裁剪参数，别为了「修」`show_cover_pic` 去重推草稿（会撞幂等守卫，强推则产生重复草稿）。
3. 真要发布前，封面显示仍以**手机预览**为准——这条自动化替代不了。
...
15. 自述性数字（读者数/粉丝数/阅读量）**不可核实就不写**——旧的「100K+ 公众号读者」已删，作者要求长期保持，不得回退（规范见 CONTENT_STANDARD §2.5）。宁可写「15 篇文章」这种能数清的。
16. 首页离线兜底快照必须自带 `stats`，否则 fetch 失败时统计会**反而变小**（实测离线只显示 6 篇而非 15 篇）——兜底不能让数字失真。
