...
- **编辑状态**：三轮审核通过，可部署
- **网站状态**：已部署并完成公开页面验证
- **公众号状态**：已于 2026-09-10 由作者人工正式发布
- **公众号草稿日期**：2026-09-08
- **公众号终稿更新**：2026-09-09（发布前复核版：删除阅读原文提示、新增两张数据图表、重做表格配色、修正 5 处引用；同日晚间修复图表比例——公众号编辑器不渲染 `td` 百分比宽度，条形改用 inline-block section 进度条实现，实测渲染比例与数值一致）
- **公众号终稿更新（二次）**：2026-09-10（草稿原地更新成功；结语后新增「如何用 AI 写公众号」分享钩子；清除外部编辑器注入的 `data-page-node-id` 属性——曾把正文撑到 26467 字符、超微信 2 万上限）
...
判断劳动是否仍属于人，不应只看“人有没有在流程里”，而应看劳动者是否拥有四项真实权利：
...
文章开头采用本次公众号自动化流程作为真实案例：

- AI 完成文章检查、HTML 清理、封面处理、草稿生成和幂等检查；
- 第一篇 MVP 草稿因文章此前已发布而不再正式发布；
- “不发”由作者在核对真实发布历史后决定；
- 不虚构人物、绩效数据、组织收益或劳动者经历。
...
1. 每条来源的可点击链接与用途说明；
2. ILO 统计口径的“暴露不等于替代”说明；
3. 减轻/放大异化的双向对照；
4. 自动化流程中的人机责任图；
5. 方法与边界、更新记录；
6. 本文的写作工具链（事实核查→网站 canonical→公众号草稿→人工确认），作为后续“如何用 AI 写公众号”的分享入口；
7. 封面原图及视觉资产入口。
...
| `article-full.html` | canonical 网站原文 |
| `article-wechat.html` | 微信公众号兼容版 |
| `poster.html` | 竖版摘要海报 |
| `cover-assets.html` | 封面与分享素材预览 |
| `assets/cover-900x383.png` | 公众号 API 上传封面 |
| `review-log.md` | 三轮审核与发布门记录 |
...
- [x] 哲学概念使用工具书或出版社页面核验
- [x] 网站版与公众号版核心观点一致
- [x] 公众号来源序号使用显式 `<span>`，不依赖自动 marker
- [x] 公众号正文无外链、脚本、样式块或正文图片
- [x] 封面为原创素材，无文字事实信息
- [x] 三轮审核连续通过
- [x] 网站已部署并核验
- [x] 公众号草稿已生成、原地更新并反查
